	...
	*/
	...
	stopFalling: function (e) {
		...
		//this._gy = -1 * this._bounce;
                //Izmenio tako da pad prestaje tek kad padne odozgo
                if(this.y < e._y)
                {
                  this._falling = false;
                  if (this._up) this._up = false;
                  this.trigger("StopFalling");
                }
	},
...
Crafty.c("Multiway", {
  ...
  _enterframe: function () {
	...
	init: function () {
		...
		this._keys = {};
                //Dodao facing
		this._movement = { x: 0, y: 0, facing: 1};
		this._speed = { x: 3, y: 3 };
	},
		...
		this.bind("EnterFrame", function () {
			...
			if (this._up) {
				...
				this._falling = true;
                        ...
                        //Dodao Jump event za skok, up: 1 == skok
		}).bind("KeyDown", function () {
			if (this.isDown("UP_ARROW") || this.isDown("W") || this.isDown("Z"))
                        {
                           this.trigger("Jump", {up: 1, facing: this._movement.facing});
                           this.trigger('Moved', { x: this.x, y: this.y - this._movement.y });
                           this._up = true;
                        }
		})
                .bind("KeyUp", function ()
                      {
                        this.trigger("Jump", {up: 0, facing: this._movement.facing});
                      });
...
function tweenEnterFrame(e) {
	...
	for (k in this._step) {
		prop = this._step[k];
		this[k] += prop.val;
                //Dodao trigger za proveru kolizije i jer je prosto dobro imati ga
                this.trigger("TweenChanged", k);
		if (--prop.rem == 0) {
